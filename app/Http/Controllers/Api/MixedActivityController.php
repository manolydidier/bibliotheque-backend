...
class MixedActivityController extends Controller
{
    public function index(Request $request, int $userId)
    {
        ...
        if ($isPrivileged) {
            $items = $items->merge(
                $this->mapGlobalFeed($userId, $asTarget, $fromDate, $toDate)
            );
        }
   ...
   private function mapPersonalActivities(int $userId, $fromDate, $toDate, string $type, string $q, int $cap): \Illuminate\Support\Collection
{
    $qBuilder = \App\Models\Activity::query()
    ...
    private function mapGlobalFeed(int $userId, bool $asTarget, $fromDate, $toDate): Collection
    {
        $items = collect();
        ...
        foreach ($roleQ as $ur) {
            ...
            $items->push([
                'id'         => 'role-'.$ur->id,
                'type'       => 'role_assigned',
                'title'      => $asTarget ? sprintf('%s t’a attribué le rôle « %s »', $by, $roleName)
    ...
    private function fetchPermissionEvents(?int $actorId = null, ?int $userAny = null): array
    {
        ...
        if (Schema::hasTable('role_permission_audits')) {
            ...
            foreach ($rows as $e) $out[] = $this->mapPermEventRow($e);
            return $out;
        }
